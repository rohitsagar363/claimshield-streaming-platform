...
from producers.scenario_data import document_uploads


def main() -> None:
    parser = argparse.ArgumentParser(description="Produce document upload events.")
    ...
    def record_batches():
        ...
        for batch_number in batches:
            records = document_uploads(batch_number=batch_number)
            if args.limit > 0:
                records = records[: args.limit]
            yield records

    produce_record_batches(
        topic="claims.documents.raw",
        schema_file="document_uploaded.json",
