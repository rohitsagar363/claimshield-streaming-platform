...
from producers.scenario_data import status_updates


def main() -> None:
    parser = argparse.ArgumentParser(description="Produce claim status events.")
    ...
    def record_batches():
        ...
        for batch_number in batches:
            records = status_updates(batch_number=batch_number)
            if args.limit > 0:
